...
let proxy = httpProxy.createProxyServer({}); // See (†)
const VIDEO_NUM = 250
const PER_PAGE_LIMIT_NUM = 20
const TOP_LIKE_COMMENT_NUM = 3
module.exports = {
    'GET /api/common/video/getVideoPath/:id': async (ctx, next) => {
        let res =  request(`http://173.82.197.114:8399/xnxx.php?x=${ctx.params.id}`);
    ...
    'GET /api/common/video/getPopularVideo': async (ctx, next) => {
        ...
        if (!isExists) {
            ...
            for (let videoId of VideoSetId) {
                let res = await db.sequelize.query(`select VideoInfo.userId,UserInfo.userAvatar,UserInfo.userNickname,VideoInfo.videoId,VideoInfo.videoCover,VideoInfo.videoStyle,VideoInfo.videoTags,videoTitleCode,VideoInfo.videoDesc,VideoInfo.videoPath from VideoInfo
        inner join UserInfo
