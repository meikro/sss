  ...
  'GET /api/common/video/getPopularVideo': async (ctx, next) => {
    ...
    if (!isExists) {
      ...
      for (let videoId of VideoSetId) {
        let res = await db.sequelize.query(`select VideoInfo.userId,UserInfo.userAvatar,UserInfo.userNickname,VideoInfo.videoId,VideoInfo.videoCover,VideoInfo.videoStyle,VideoInfo.videoTags,VideoInfo.videoDesc,VideoInfo.videoPath from VideoInfo
        inner join UserInfo
